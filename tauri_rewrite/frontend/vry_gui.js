...
let tauriInvoke =
  _tauriInvoke || (() => Promise.reject(new Error("IPC unavailable")));
let tauriListen = _tauriListen || (() => Promise.resolve(() => {}));
if (!_tauriInvoke && !_tauriListen) {
  console.warn(
    "[VRY] Tauri IPC global not detected; the app appears to be running outside the Tauri runtime. Backend-dependent features will be unavailable.",
  );
}
...
(function () {
  ...
  /** @param {string} t */
  let stripHint = function (t) {
    return t ? t.replace(COPY_HINT, "") : "";
  };
  let chromaColor = function (/** @type {string|undefined} */ name) {
    let m = name && name.match(/\(Variant \d+ (.+)\)$/);
    return m ? m[1] : "";
  };
  ...
  /** @param {*} v @param {*} [fb] */
  function txt(v, fb) {
    return isEmpty(v) ? (fb === undefined ? NA : fb) : String(v);
  }
  ...
  function rankColor(idx) {
    let n = Number(idx);
    if (isEmpty(idx) || Number.isNaN(n) || n < 0 || n >= RANK_COLORS.length)
      return null;
  ...
  function buildStatChips(player, chipClass, shortName, compSuffix) {
    ...
    setStatChip(
      frag,
      "Win Rate" + comp,
      winRateDisplay(player.winPercentage),
      isEmpty(player.winPercentage),
      chipClass,
    );
    setStatChip(
      frag,
      "Rank",
      rankName(player.rank, shortName),
      isEmpty(player.rank),
      chipClass,
      rankColor(player.rank),
      ri && player.rank != null ? ri[player.rank] : undefined,
    );
    setStatChip(frag, "RR", txt(player.rr), isEmpty(player.rr), chipClass);
    setStatChip(
      frag,
      "Leaderboard",
      isEmpty(player.leaderboard)
        ? NA
        : Number(player.leaderboard) <= 0
          ? NA
          : "#" + player.leaderboard,
      isEmpty(player.leaderboard),
      chipClass,
    ...
    setStatChip(
      ...
      "Peak Rank",
      (function (rn) {
        return rn !== NA && player.peakRankAct
          ? rn + String(player.peakRankAct).trim()
          : rn;
      })(rankName(player.peakRank, shortName)),
      isEmpty(player.peakRank),
      chipClass,
      rankColor(player.peakRank),
      ri && player.peakRank != null ? ri[player.peakRank] : undefined,
    );
    setStatChip(
      frag,
      "Last Act",
      rankName(player.previousRank, shortName),
      isEmpty(player.previousRank),
      chipClass,
      rankColor(player.previousRank),
      ri && player.previousRank != null ? ri[player.previousRank] : undefined,
    );
    setStatChip(
      frag,
      "Level",
    ...
    setStatChip(
      frag,
      "Last Active" + comp,
      txt(player.lastActive),
      isEmpty(player.lastActive),
      chipClass,
    );
    return frag;
  }
  ...
  async function takeDetailScreenshot() {
    ...
    if (!target || target.hidden) {
      showToast("No player loadout open.");
      return;
    }
  ...
  function setupTauriListeners() {
    tauriListen("heartbeat", function (event) {
      try {
        ...
        let evSession =
          event.payload && typeof event.payload.sessionId === "number"
        ...
        console.error("[VRY] heartbeat handler error:", e);
      }
    });
    ...
    tauriListen("rank_icons", function (event) {
      try {
        ...
        console.error("[VRY] rank_icons handler error:", e);
      }
    });
    ...
    tauriListen("state_change", function (event) {
      try {
        ...
        console.error("[VRY] state_change handler error:", e);
      }
    });
    ...
    tauriListen("backend_ready", function (event) {
      try {
        ...
        console.error("[VRY] backend_ready handler error:", e);
      }
    });
    ...
    tauriListen("riot_client_launching", function () {
      try {
        ...
        console.error("[VRY] riot_client_launching handler error:", e);
      }
    });
    ...
    tauriListen("riot_client_waiting", function () {
      try {
        ...
        console.error("[VRY] riot_client_waiting handler error:", e);
      }
    });
    ...
    tauriListen("cache_cleared", function (event) {
      try {
        ...
        console.error("[VRY] cache_cleared handler error:", e);
      }
    });
    ...
    tauriListen("log_update", function (event) {
      try {
        let line =
          /** @type {string} */ (/** @type {unknown} */ (event.payload)) || "";
        logLines.push(line);
        ...
        console.error("[VRY] log_update handler error:", e);
      }
    });
    ...
    tauriListen("auth_error", function (event) {
      try {
        let message =
          (event.payload && String(event.payload.message)) ||
          "Please sign in to Riot Client and click Refresh.";
        showAuthErrorModal(message);
      } catch (e) {
        console.error("[VRY] auth_error handler error:", e);
      }
    });
  }
  ...
  function bumpTimestampOnly(payload) {
    ...
    if (chip)
      chip.textContent =
        "Updated " + new Date(payload.time * 1000).toLocaleTimeString();
  }
  ...
  function setPayload(payload) {
    try {
      ...
      if (
        state.selectedPuuid &&
        state.matchPuuid !== undefined &&
        newMatchPuuid !== state.matchPuuid
      ) {
  ...
  function normalizePlayers(payload) {
    ...
    return players
      .filter(function (p) {
        return p.name || p.agent || p.weapons;
      })
      .sort(function (a, b) {
        // Self first
  ...
  function scheduleGridRender() {
    ...
    requestAnimationFrame(function () {
      ...
      if (state.dirty.meta) {
        renderMeta();
        state.dirty.meta = false;
      }
      if (state.dirty.players) {
        renderPlayers();
        state.dirty.players = false;
      }
    });
  }
  ...
  function render() {
    scheduleGridRender();
    renderTopLinks();
    if (state.dirty.playedWith) {
      renderPlayedWith();
      state.dirty.playedWith = false;
    }
    ...
    if (state.dirty.json) {
      renderJson();
      state.dirty.json = false;
    }
  }

  function renderTopLinks() {
    let self = state.players.filter(function (p) {
      return p.isSelf;
    })[0];
  ...
  function updateSelection() {
    ...
    state.playerButtons.forEach(function (btn) {
      btn.classList.toggle(
        "is-selected",
        btn.dataset.puuid === state.selectedPuuid,
      );
    });
  }
  ...
  function handlePlayerGridClick(e) {
    try {
      ...
      let button = /** @type {HTMLElement|null} */ (
        target && target.closest(".player-button")
      );
      if (!button) return;
  ...
  function handleDelegatedContextMenu(e) {
    try {
      ...
      if (node && node.nodeType === 3)
        node = /** @type {Node} */ (node.parentNode) || node; // text node
      let el = null;
      while (node && node !== document && node.nodeType === 1) {
        if (
          /** @type {HTMLElement} */ (node).title &&
          /** @type {HTMLElement} */ (node).title.indexOf(COPY_HINT) !== -1
        ) {
          el = /** @type {HTMLElement} */ (node);
          break;
        }
      ...
      copyToClipboard(text)
        ...
        .catch(function (e) {
          console.error("[VRY] Context menu copy failed:", e);
  ...
  function handleGridImageError(e) {
    try {
      if (
        ...
        /** @type {HTMLElement} */ (e.target).classList.contains("agent-avatar")
      ) {
  ...
  function showLoadingChip(label) {
    ...
    chip.append(
      spinner,
      document.createTextNode("Loading " + label + " Data\u2026"),
    );
  ...
  function renderStateTransition(newState) {
    ...
    let isPregameToIngame =
      state.prevGameState === "PREGAME" && newState === "INGAME";
    ...
    let keepUi =
      isPregameToIngame || (newState === state.lastGameState && state.payload);
    if (keepUi) {
    ...
    loadMsg.append(
      loadSpinner,
      document.createTextNode(" Waiting for " + label + " data\u2026"),
    );
  ...
  function renderMeta() {
    ...
    let st = p.state || "";
    segments.push({
      text: STATE_LABELS[st] || txt(p.state, "Unknown"),
      cls: STATE_CLASSES[st] || "",
    });
    ...
    if (p.time) {
      ...
      segments.push({
        text: "Updated " + d.toLocaleTimeString(),
        cls: "updated",
        id: "metaUpdatedChip",
      });
    }
  ...
  function renderPlayers() {
    ...
    state.players.forEach(function (player) {
      ...
      button.classList.toggle(
        "is-selected",
        player.puuid === state.selectedPuuid,
      );
      ...
      let avatar = buildAgentAvatar(
        player.agentImgLink,
        player.agent,
        player.agentSelectionState,
      );
      ...
      let rankIconUrl =
        riMap && player.rank != null ? riMap[player.rank] : undefined;
      if (rankIconUrl) {
        let ri = document.createElement("img");
        ri.className = "rank-icon";
      ...
      button.append(
        avatar,
        identity,
        buildCardStats(player),
        buildPreviewRow(player),
      );
  ...
  function renderTeamHeaders() {
    ...
    let hasBlue = state.players.some(function (p) {
      return p.team === "Blue";
    });
    let hasRed = state.players.some(function (p) {
      return p.team === "Red";
    });
    ...
    if (singleTeam) {
      ...
      els.atkSection.hidden = true;
      els.defSection.hidden = false;
      els.defHeader.textContent =
        (isMyTeam ? "ALLY" : "ENEMY") + " (" + teamSide(onlyTeam) + ")";
      ...
      els.defHeader.textContent =
        (leftTeam === selfTeam ? "ALLY" : "ENEMY") +
        " (" +
        teamSide(leftTeam) +
        ")";
      els.atkHeader.textContent =
        (rightTeam === selfTeam ? "ALLY" : "ENEMY") +
        " (" +
        teamSide(rightTeam) +
        ")";
  ...
  function buildPreviewRow(player) {
    ...
    PREVIEW_WEAPONS.forEach(function (weaponName) {
      ...
      let skinLabel = weapon
        ? weapon.skinDisplayName || weapon.weapon || weaponName
        : NA;
  ...
  function renderDetails() {
    let selected = state.players.find(function (p) {
      return p.puuid === state.selectedPuuid;
    });
    ...
    if (!hasSelection && state.players.length === 0) {
      els.emptyStateTitle.textContent = "No match data";
      els.emptyStateText.textContent = "Waiting for VRY backend data.";
    } else if (!hasSelection) {
      els.emptyStateTitle.textContent = "No player selected";
      els.emptyStateText.textContent =
        "Click a player card to view loadout and stats.";
    }
    if (!hasSelection) {
    ...
    els.selectedAgent.classList.toggle(
      "is-selecting",
      selected.agentSelectionState === "selected",
    );
    // Censor Self Player Name
    els.selectedName.textContent = txt(
      selected.isSelf ? "You" : selected.name,
      "Unknown Player",
    );
    ...
    els.selectedLevel.textContent = isEmpty(selected.level)
      ? "Level " + NA
      : "Level " + selected.level;
    els.selectedTeam.textContent = selected.team
      ? myTeam(state.payload) === selected.team
        ? "ALLY"
        : "ENEMY"
      : "Unknown";
    ...
    if (selected.playerCard) {
      els.playerCardPreview.style.backgroundImage =
        'url("' + safeHttps(selected.playerCard) + '")';
      els.playerCardPreview.title =
        (selected.playerCardName || "Player Card") + COPY_HINT;
    } else {
      els.playerCardPreview.style.backgroundImage = "";
  ...
  function renderExpressions(player) {
    ...
    slots.forEach(function (expression, idx) {
      ...
      let iconSrc =
        expression &&
        (expression.fullTransparentIcon || expression.displayIcon);
      if (iconSrc) {
        let img = document.createElement("img");
    ...
    WEAPON_COLUMNS.forEach(function (column) {
      ...
      column.groups.forEach(function (group) {
        ...
        group.weapons.forEach(function (weaponName) {
          gridFrag.append(buildWeaponTile(player, weaponName));
        });
  ...
  function buildWeaponTile(player, weaponName) {
    ...
    tile.title =
      (weapon
        ? weaponName +
          ": " +
          (weapon.skinDisplayName || weapon.weapon || "Unknown skin") +
          vc
    ...
    let iconSrc =
      weapon && (weapon.skinDisplayIcon || weapon.weaponDisplayIcon);
    if (iconSrc) {
      let img = document.createElement("img");
      img.src = safeHttps(iconSrc);
      img.decoding = "async";
  ...
  function renderPlayedWith() {
    ...
    let payload = state.payload;
    let entries =
      payload && Array.isArray(payload.alreadyPlayedWith)
        ? payload.alreadyPlayedWith
        : [];
    ...
    entries.forEach(function (entry) {
      ...
      nc.textContent = txt(entry.name, "Unknown");
      row.append(nc);
      let rel = entry.relation === "ally" ? "Ally" : "Enemy";
      let cc = document.createElement("td");
      cc.textContent = rel + " " + txt(entry.agent, "Unknown");
      row.append(cc);
      let tc = document.createElement("td");
      tc.textContent = formatEncounterTimes(entry);
      row.append(tc);
      ...
      lc.textContent =
        ...
        lastMap +
        " \u2014 " +
        formatTimeAgo(/** @type {string|number} */ (entry.time_diff)) +
        " ago";
      row.append(lc);
      let rc = document.createElement("td");
      rc.textContent = formatEncounterRecord(entry);
      row.append(rc);
      frag.append(row);
    });
  ...
  function copyText(text, button) {
    ...
    let done = function () {
      ...
      // @ts-expect-error ??? _copyT is an expando on the button
      button._copyT = setTimeout(function () {
    ...
    copyToClipboard(text)
      .then(done)
      .catch(function (e) {
        console.error("[VRY] copyText error:", e);
  ...
  function fallbackCopy(text, done) {
    ...
    try {
      ...
      console.error("[VRY] execCommand copy failed:", e);
      showToast("Copy failed.");
    }
    ta.remove();
  }
  ...
  els.detailsPanel.addEventListener("click", function (e) {
    if (e.target === els.detailsPanel) {
      deselectPlayer();
    }
  });
  window.addEventListener("keydown", function (e) {
    if (e.key === "Escape") {
      ...
      if (state.selectedPuuid) {
        deselectPlayer();
      }
    }
  });
  window.addEventListener("auxclick", function (e) {
    if (e.button === 3 && state.selectedPuuid) {
      e.preventDefault();
      deselectPlayer();
    }
  });
  ...
  els.jsonCopyBtn.addEventListener("click", function () {
    copyJson(els.jsonCopyBtn);
  });
  ...
  els.copyStatsBtn.addEventListener("click", function () {
    try {
      let payload = state.payload;
      let p =
        payload && state.selectedPuuid && payload.players
          ? payload.players[state.selectedPuuid]
          : null;
      if (!p) {
        showToast("Nothing to copy yet.");
        return;
      ...
      let parts = [
        ...
        "RR: " + txt(p.rr),
        "Leaderboard: " +
          (isEmpty(p.leaderboard) || Number(p.leaderboard) <= 0
            ? NA
            : "#" + p.leaderboard),
        "Peak Rank: " +
          (peakStr !== NA && p.peakRankAct
            ? peakStr + String(p.peakRankAct).trim()
            : peakStr),
      ...
      copyToClipboard(parts.join(" | "))
        .then(function () {
          showToast("Copied: Stats");
        })
        .catch(function (e) {
          console.error("[VRY] Stats copy failed:", e);
          showToast("Failed.");
        });
    } catch (e) {
  ...
  if (logOpenBtn) {
    logOpenBtn.addEventListener("click", function () {
      tauriInvoke("open_log_file").catch(function (e) {
        console.error("[VRY] open_log_file failed:", e);
        showToast("Failed to open log file");
      });
    });
  }
  ...
  if (hbOpenBtn) {
    hbOpenBtn.addEventListener("click", function () {
      tauriInvoke("open_heartbeat_file").catch(function (e) {
        console.error("[VRY] open_heartbeat_file failed:", e);
        showToast("Failed to open heartbeat file");
      });
    });
  }
  ...
  [
    ...
    },
    {
      ...
      pre: els.hbPre,
      copy: els.hbCopyBtn,
      refresh: els.hbRefreshBtn,
      render: renderHeartbeatTail,
    },
  ].forEach(function (p) {
    p.panel.addEventListener("toggle", function () {
      if (p.panel.open) p.render();
    });
    p.copy.addEventListener("click", function () {
      copyText(p.pre.textContent, p.copy);
    });
  ...
  if (els.hbPanel) {
    els.hbPanel.addEventListener("toggle", function () {
      if (els.hbPanel.open) renderJson();
    });
  }
  ...
  if (els.playedWithPanel) {
    els.playedWithPanel.addEventListener("toggle", function () {
      if (els.playedWithPanel.open) renderPlayedWith();
    });
  }
