...
**Rust repo:** `VRY_rewrite_tauri_fixed/tauri_rewrite` (31 .rs files, ~3800 lines)  
**Python version:** 2.99  
**Rust version:** 3.0.0
...
| State transition: INGAME→other | Queues match result with retries (`queue_match_result_update` + `process_pending_match_results`) | Updates match result directly (`update_match_result` on state transition) | ❌ No retry queue |
...
| Rank caching | `requestMap` dict with TTL | `Mutex<HashMap<String, (PlayerRank, Instant)>>` with 300s TTL | ✅ |
...
| Match details | `GET /match-details/v1/matches/{match_id}` from **Pd** | Same, from **Pd** (`UrlType::Pd`) — **FIXED: was `UrlType::Glz`** | ✅ |
...
| Last active epoch | `(MatchStartTime + gameLengthMillis) / 1000` — **match END time** | Same: `(match_start_time + game_length_millis) / 1000` — **FIXED** | ✅ |
...
| Conditional fetch | `_should_fetch_comp_stats()` checks table flags (skips API if all stats columns hidden) | **Always fetches** — does not check table flags | ❌ |
...
| Match result retry queue | `queue_match_result_update` + `process_pending_match_results` (retries on failure) | Not implemented (updated directly on INGAME→other transition) | ❌ |
...
| All summaries (MENUS) | N/A (not used in Python frontend) | `get_all_summaries()` for MENUS state — `alreadyPlayedWith` in heartbeat | ✅ |
| Match result details | Checks multiple key casing variants (`teamId`/`TeamID`/`teamID`), falls back to comparing scores | Handles `matchInfo.winningTeam` + `teams[].won` bool + multiple casing variants | ✅ |
...
| JS engine | `docs/vry_gui.js` (~1295 lines) | Same `vry_gui.js` (~853 lines) | ⚠️ Rust frontend more compact |
...
**Note:** Python uses `flags.short_ranks` to abbreviate rank names in the table. Rust stores the flag but the frontend/backend doesn't implement the abbreviated rendering. ⚠️
...
|----------|--------|-----------|---------|
| Party number MENUS | partyNumber=1 for self, 0 for others | partyNumber=1 for ALL players | ⚠️ Minor diff (frontend ignores 0 anyway) |
| Party number INGAME | partyNumber=1 for self+party, 0 for others | partyNumber=0 for ALL players | ⚠️ Minor diff |
| Conditional stats fetching | Skips MMR API if all stat columns hidden | Always fetches competitive updates + match details | ⚠️ Extra API calls |
...
|----------|--------|------|
| API client | 308 (requestsV.py) | 275 (api/client.rs) + 148 (api/auth.rs) |
| State machine / main loop | 1357 (main.py) | 286 (core/state_machine.rs) + 749 (core/payload_builder.rs) |
| Service layer | 155 player_stats.py + 147 rank.py + 42 names.py + 118 presences.py + 232 stats.py + 295 Loadouts.py | 188 stats.rs + 172 rank.rs + 116 names.rs + 183 presences.rs + 257 loadouts.rs + 302 encounters.rs + 239 config.rs + 80 logging.rs |
| Data models | ~200 (inline in various files) | 211 mmr.rs + 168 loadout.rs + 306 content.rs + 68 auth.rs + 49 presences.rs + 194 match_data.rs + 92 heartbeat.rs |
| Frontend JS | 1295 (vry_gui.js) | 853 (vry_gui.js) |
| Frontend HTML/CSS | ~800 | ~800 |
| **Total** | **~5391** | **~3800 (Rust) + ~1650 (JS/CSS) = ~5450** |
...
- ⚠️ **Minor behavioral diffs**: No buddy_level socket UUID, no conditional stats fetching, party number always 1 in MENUS, no Deceive detection, polling-based (not WS) presences, no retry queue for match results, missing short_ranks rendering, no skin tier color.
...
- **Key fixes applied**: Match-details endpoint URL type (`Glz`→`Pd`), `last_active` epoch calculation (added `gameLengthMillis`), encounter match result fallback for winning team, icon fallback for sprays/flex, `PlayerCardName`/`TitleName` fields.
